...
func log(Breakfast: String) -> Meal? {
    if let meal = meals[Breakfast] {
        return meal
    } else {
        return nil
    }
...
if let meal = log(Breakfast: "Breakfast") {
    print(meal)
}
/*:
 ...
 */
func checkSave() -> [String: Any] {
    if let save = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return save
    } else {
        return [:]
    }
